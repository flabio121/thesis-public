...
\begin{enumerate}
\item save COMSOL J--V and state tables under
...
The COMSOL Global Evaluation sweep for $N_{t,0}$, $R_{s,0}$, and $R_{sh,0}$
is processed separately from the aging-time pipeline:
\begin{verbatim}
python .\scripts\plot_comsol_parameter_sweep.py
\end{verbatim}
This command reads the architecture-scoped parameter-sweep data under
...
\end{verbatim}
The current application uses placeholder prediction logic and COMSOL seed
tables. Its purpose is to define the future data structure, provenance record,
analysis bundle, inverse-diagnosis interface, and active-learning workflow
before the full COMSOL design of experiments is available.

\section{Architecture-Stress Grid Workflow}

The architecture-scoped aging data are processed with:
\begin{verbatim}
...
baseline p-i-n temperature-aging data. The second command summarizes the
