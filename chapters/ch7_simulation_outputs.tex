...
held parameters are fixed at the best-performing baseline values:
$N_{t,0}=10^{14}~\mathrm{cm^{-3}}$, $R_{s,0}=0.1~\Omega\mathrm{cm^2}$, and
$R_{sh,0}=10^{6}~\Omega\mathrm{cm^2}$.}
...
19.44--15.91\% &
FF decreases from 0.784 to 0.642, consistent with series loss. \\
$R_{sh,0}$ &
$10^{3}$--$10^{6}~\Omega\mathrm{cm^2}$ &
18.39--19.44\% &
FF improves from 0.741 to 0.784 as leakage is suppressed. \\
\bottomrule
\end{tabular}
\end{table}
...
scalar photovoltaic metrics. In this export, the best-performing baseline point
has PCE $=19.44\%$, $V_{oc}=1.205$ V, $J_{sc}=20.58~\mathrm{mA/cm^2}$, and
FF $=0.784$. The all-combination sweep spans 13.42--19.44\% PCE. This range is
...
parameter sweeps to architecture-scoped aging outputs. The first validated
architecture-stress J--V export uses the baseline p-i-n architecture at one sun
and varies aging temperature from 25 to 85~$^\circ$C. Each condition contains
ten diagnostic J--V scans at equivalent aging times
0, 50, 100, 200, 300, 400, 500, 600, 800, and 1000 h. These scans use the
same device architecture and processing convention as the internal-state
profile grid described below, but they should still be interpreted as COMSOL
accelerated-aging outputs rather than direct field lifetimes.

\input{outputs/tables/comsol_architecture_stress_grid/baseline_pin_stress_matrix_jv_001_summary}

\begin{figure}[H]
\centering
\includegraphics[width=0.86\textwidth]{outputs/figures/comsol_architecture_stress_grid/baseline_pin_stress_matrix_jv_001_pce_retention.png}
\caption{PCE retention from the validated one-sun baseline p-i-n temperature-aging J--V export. Higher temperature accelerates the modeled degradation: the 1000 h retention decreases from 97.6\% at 25~$^\circ$C to 70.0\% at 85~$^\circ$C.}
\label{fig:baseline_pin_temperature_pce_retention}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{outputs/figures/comsol_architecture_stress_grid/baseline_pin_stress_matrix_jv_001_pv_metrics.png}
\caption{Extracted PCE, fill factor, $J_{sc}$, and $V_{oc}$ for the validated one-sun temperature-aging export. The dominant response at high temperature is a coupled loss in $V_{oc}$ and FF, with a smaller but visible decrease in $J_{sc}$.}
\label{fig:baseline_pin_temperature_pv_metrics}
\end{figure}

The temperature sweep provides a cleaner aging trend than the isolated
electronic sweep because the same latent degradation model is evolved through
time. The final 85~$^\circ$C case has lower $V_{oc}$, lower FF, and lower
$J_{sc}$ than the lower-temperature cases, consistent with stronger
temperature-accelerated trap and resistance evolution. The result is still not
a full calibration because the COMSOL aging time is an accelerated equivalent
time and because the current model has not yet been fit to a balanced
experimental temperature matrix.

The later flat J--V export,
...
Table~\ref{tab:profile_grid_extrema} show that the internal-state grid has the
expected stress ordering. The smallest final trap response occurs at the
lowest light and temperature, while the largest final trap and effective
trap-density responses occur at 400 K and one sun. The resistance-damage states
increase by orders of magnitude with temperature but are comparatively weakly
dependent on light in this parameterization. This distinction is useful for
future inverse modeling: a terminal J--V curve alone may not separate trap
growth from resistance evolution, but a COMSOL-generated state grid can define
the physically plausible state manifold for surrogate training.
...
One uploaded low-light J--V export,
\path{data/raw/comsol/arch_baseline_pin/Aging/Stress_matrix_jv_002.csv}, is
retained in the repository but is not used as a quantitative result. The file
is labeled as a 0.01-sun aging-temperature sweep with one-sun diagnostic scans,
but the exported current density is approximately
$-3.8\times10^5~\mathrm{mA/cm^2}$ across the voltage range. As a result, the
curves do not cross zero current and the post-processing script correctly
flags all extracted $V_{oc}$, FF, and PCE values as invalid. The placeholder
below defines the corrected export needed to complete the low-light J--V
portion of the architecture-stress matrix.

\begin{figure}[H]
\centering
\fbox{\parbox[c][1.35in][c]{0.84\textwidth}{\centering
\textbf{DATA PLACEHOLDER: corrected low-light J--V export}\\
Re-export \texttt{Stress\_matrix\_jv\_002.csv} with current density on the
physical scale of roughly $-40$ to $+25~\mathrm{mA/cm^2}$, separate
\texttt{aging\_light\_suns} and \texttt{scan\_light\_suns} labels, and the same
ten diagnostic aging checkpoints used by the validated one-sun sweep.}}
\caption{Placeholder for the corrected low-light temperature-aging J--V export.}
\label{fig:placeholder_low_light_jv_export}
\end{figure}
...
$r_{\mathrm{scan}}=0.1~\mathrm{V/s}$. It does not prove that the selected ion
mobility or concentration is unique.

\section{Pending Spatial and Transient Validation Results}

The presentation identifies three additional outputs needed for a complete
device-physics validation: simulated band diagrams, publication-ready
time-resolved mobile-ion and trap line profiles, and a quantitative DIT
transient. The profile-grid summary in
Fig.~\ref{fig:profile_grid_final_state_summary} establishes that the internal
state variables can be exported and summarized across stress conditions, but a
small set of polished line profiles is still needed to show spatial shape,
interface localization, and conservation directly. The governing equations and
extraction methods are implemented or documented, but the following
placeholders remain before making device-specific validation claims.

\begin{figure}[H]
\centering
\fbox{\parbox[c][1.35in][c]{0.84\textwidth}{\centering
\textbf{RESULT PLACEHOLDER: Band diagrams}\\
Export $E_c$, $E_v$, electron and hole quasi-Fermi levels, and electrostatic
potential for equilibrium, short circuit, maximum power, and open circuit.
Confirm carrier-selective alignment and the ITO-to-Ag coordinate direction.}}
\caption{Placeholder for architecture-consistent simulated band diagrams.}
\label{fig:placeholder_band_diagrams}
\end{figure}

\begin{figure}[H]
\centering
\fbox{\parbox[c][1.35in][c]{0.84\textwidth}{\centering
\textbf{RESULT PLACEHOLDER: spatial line profiles}\\
Select representative low-stress and high-stress cases from the profile grid
and plot $u_{\mathrm{ion}}(x,t)$, conserved absorber-average ion concentration,
$D_{\mathrm{trap}}(x,t)$, and $N_{t,\mathrm{eff}}(x,t)$ at early and late
aging times.}}
\caption{Placeholder for ion/trap spatial line profiles.}
\label{fig:placeholder_ion_trap_profiles}
\end{figure}

\begin{figure}[H]
\centering
\fbox{\parbox[c][1.35in][c]{0.84\textwidth}{\centering
\textbf{RESULT PLACEHOLDER: DIT extraction}\\
Plot the voltage pulse, raw and frozen-ion control currents,
baseline-corrected ionic current, integrated charge, and extracted
$N_0=Q_{\mathrm{ion}}/(qAL)$.}}
\caption{Placeholder for quantitative DIT current integration and mobile-ion concentration extraction.}
\label{fig:placeholder_dit_result}
\end{figure}
