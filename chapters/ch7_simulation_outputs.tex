...
electronic parameter sweeps to architecture-scoped aging outputs. The canonical
baseline p-i-n light-temperature J--V export stores a full diagnostic J--V
sweep as a concatenated two-column table. The post-processing script splits
this file into 360 J--V curves using the COMSOL parameter ordering shown in the
export: ten aging
...
crossing. Superseded development exports are not used for the quantitative
...
\caption{Conceptual COMSOL-to-ML workflow. The diagram separates raw COMSOL
exports, canonical parsing, feature engineering, grouped validation, and
traceable thesis artifacts. Quantitative surrogate results are generated by
the reproducible scripts rather than by this conceptual artwork.}
\label{fig:ml_concept_workflow}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{figures/ml_surrogate/aging_doe_trajectory_concept.png}
\caption[Conceptual light-temperature-aging DOE map.]{Conceptual light-temperature-aging design-of-experiments map. The
canonical stress matrix samples light intensity, aging temperature, and
equivalent aging time; the high-stress trajectory is shown as the most severe
aging path in the grid.}
\label{fig:ml_concept_doe}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figures/ml_surrogate/forward_surrogate_architecture_concept.png}
\caption{Conceptual forward COMSOL-trained surrogate architecture. The thesis
trains only the forward lite surrogate; the architecture is framed as a
traceable interpolation layer over COMSOL-generated stress-grid data.}
\label{fig:ml_concept_forward_architecture}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figures/ml_surrogate/inverse_diagnosis_workflow_concept.png}
\caption[Planned inverse-diagnosis workflow.]{Planned inverse-diagnosis workflow. This figure is a future-work
roadmap: inverse latent-state inference requires broader COMSOL state labels,
matched experimental aging data, and uncertainty-aware validation before it can
be treated as a calibrated diagnostic model.}
\label{fig:ml_concept_inverse_roadmap}
\end{figure}
...
columns are derived from explicit COMSOL export metadata: light intensity,
aging temperature, equivalent aging time, and, for the curve branch, applied
voltage. Second, data leakage is avoided by grouping validation folds by
...
the baseline p-i-n 6 $\times$ 6 light-temperature export used throughout this
chapter. Superseded development exports are retained only for audit history in
the repository documentation and are not interpreted as parallel quantitative
results. The current remaining data-quality limitation is the voltage range of
the canonical export: many mild or early-aging curves do not cross zero current
...
The selected COMSOL profile exports include the band-edge and quasi-Fermi-level
...
\caption{Selected COMSOL band diagrams from the profile-grid exports.
...
Figure~\ref{fig:band_diagrams_selected_cases} should not be read as a
large-contrast degradation plot. The selected cases differ, but the extracted
band-edge shifts are modest, with maximum differences on the order of
$0.03$--$0.07$ eV across the plotted stress conditions. The figure is therefore
most useful as a coordinate, layer-side, and quasi-Fermi-level consistency
check; the stronger degradation contrast is carried by the terminal J--V
metrics and the spatial damage-state profiles.
...
directly. These profiles support the qualitative interpretation that aging is
...
because these spatial exports can be variable- or domain-selected in COMSOL,
and the plotted profiles should not imply that perovskite-only trap variables
are active outside the perovskite domain.}
...
switching transient, so the export is suitable as a background-control data
product. It does not by itself define a final mobile-ion concentration because
