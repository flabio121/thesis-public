...
Aging, hysteresis, and transient-ion measurements operate on different
timescales. This chapter separates the slow stress protocol from the faster
J--V scan and from short DIT-style voltage pulses, so that voltage history,
measurement, and degradation are not treated as the same process.
...
For the aging studies reported in this thesis, the accelerated degradation
terms are parameterized so that
\begin{equation}
1~\mathrm{s~of~simulated~aging}=1~\mathrm{h~of~equivalent~aging}.
\end{equation}
Equivalently,
\begin{equation}
t_{\mathrm{age,eq}}[\mathrm{h}]=t_{\mathrm{sim}}[\mathrm{s}],
\end{equation}
and the corresponding physical acceleration is
\begin{equation}
\mathrm{DegAccel}=\frac{3600~\mathrm{s~real}}{1~\mathrm{s~sim}}=3600.
\end{equation}
This convention is a reporting and calibration choice: it makes the time axis
read directly as equivalent aging hours while keeping the transient COMSOL
problem computationally tractable. It is not a field-lifetime prediction.
The degradation-rate prefactors still need to be calibrated against matched
experimental aging data before the equivalent-time axis can be interpreted as a
device-specific lifetime prediction.
...
The Dark Injection Transient (DIT) protocol provides an ion-sensitive
complement to multi-rate J--V scans. In a DIT measurement, the device is first
placed in a defined electrical state, then a short voltage pulse is applied in
the dark so that the resulting current transient can be used to estimate
mobile-ion motion. In this thesis, a preconditioned ionic profile is perturbed
with a short voltage pulse, the terminal current transient is recorded, and the
mobile-ion charge associated with the transient component is integrated
