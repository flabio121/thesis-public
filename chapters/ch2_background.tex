...
Before degradation can be interpreted, the model must define what physical
behavior needs to be represented. This chapter summarizes the p-i-n device
operation, the meaning of J--V observables, and the electronic, ionic, optical,
and recombination mechanisms needed to study perovskite solar-cell degradation.
...
\cite{Khenkin_2020}. A model must connect an observed electrical change to a
plausible internal cause without hiding uncertainty.
...
stack. In this context, drift means charge motion driven by an electric field,
and diffusion means charge motion driven by concentration gradients. Recent PSC
modeling reviews emphasize that these models are most reliable when
assumptions are explicit and when electronic and ionic degrees of freedom are not confused
\cite{Singh_2025}. COMSOL is used here as the finite-element environment for
that device model, with the assumptions and couplings stated explicitly.
...
required for mechanism-aware interpretation. This thesis therefore treats
J--V metrics as observables to be explained by internal physical mechanisms,
not as independent degradation mechanisms.
...
Beer--Lambert generation profile. Beer--Lambert absorption is the standard
exponential attenuation of light as it travels through an absorbing material;
here it supplies a spatially varying source term to the semiconductor
equations. The photon flux entering the absorber is
represented as
\begin{equation}
