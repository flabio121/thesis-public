...
The COMSOL model translates the laboratory p-i-n device into a reduced
one-dimensional simulation domain. This chapter defines that geometry and then
describes the coupled semiconductor equations, optical generation model,
mobile-ion transport equation, degradation coupling, and J--V metric
extraction.

\section{COMSOL Multiphysics Setup}

COMSOL Multiphysics is a finite-element tool for solving coupled partial
differential equations on a shared geometry. The model used for this thesis
was tested with COMSOL Multiphysics 6.4; earlier COMSOL versions should be
rechecked before reproducing the coupled PDE setup.
In this work, the Semiconductor interface solves electronic drift-diffusion and
Poisson electrostatics, a user-defined PDE solves mobile-ion transport,
additional PDE or global states represent degradation, and the Electrical
Circuit interface represents lumped series and shunt pathways. This
organization keeps the conservation laws, domain selections, boundary
conditions, and coupling variables inspectable.
...
Contacts are applied as electrical boundaries. The UV resin and cover glass are
encapsulation layers only; they are included in the physical stack description
but are not treated as semiconductor, transport, or electrode domains in the
electronic COMSOL model. The
...
Some legacy model labels, such as ``FTO/Contact'' and ``Au/Contact,'' do not
define the physical device used in this thesis. In the results reported here,
these contact selections are interpreted as the illuminated ITO contact and
rear Ag contact. A final device-specific calibration should use consistent
stack labels, contact selections, illumination direction, and contact work
functions throughout the model.
...
The coupled model should be read as a closed feedback loop. Electrostatics
drives electronic transport and mobile-ion migration, ion redistribution feeds
back through the electrostatic source term, and degradation states modify
recombination and transport parameters. The main outputs are J--V curves,
hysteresis, internal electric fields, and aging trends.
...
The corresponding implementation variables are listed in
Tables~\ref{tab:beer_lambert_parameters} and
\ref{tab:beer_lambert_variables}. The current model uses the same
Beer--Lambert depth dependence as a normalized algebraic shape so that
...
\texttt{xpvk\_clip} & \texttt{min(max(xpvk,0[m]),Lgen)} & Clipped local coordinate; bracketed unit is COMSOL syntax \\
\texttt{Gshape\_raw} &
\begin{tabular}[t]{@{}l@{}}
...
= -D_{\mathrm{ion}}\nabla c_{\mathrm{ion}}
+D_{\mathrm{ion}}\frac{z_{\mathrm{ion}}q}{k_{\mathrm{B}}T}
c_{\mathrm{ion}}\mathbf{E}.
\label{eq:ion_flux}
\end{equation}
where $\mathbf{E}=-\nabla\phi$. Equivalently, in potential form,
\begin{equation}
\mathbf{N}_{\mathrm{ion}}
=
-D_{\mathrm{ion}}
...
\right].
\end{equation}
Here $D_{\mathrm{ion}}$ is ion diffusivity in m$^2$/s,
$z_{\mathrm{ion}}$ is the signed ion valence, and
$\mathbf{N}_{\mathrm{ion}}$ has units of m$^{-2}$s$^{-1}$. If an electrical
mobility is reported, it is related by the Einstein relation
$\mu_{\mathrm{ion}}=D_{\mathrm{ion}}q/(k_{\mathrm{B}}T)$ for singly charged
ions, or more generally the drift factor is
$D_{\mathrm{ion}}z_{\mathrm{ion}}q/(k_{\mathrm{B}}T)$. This convention avoids
double-counting $q$ in the mobility term and must be matched to the sign of
$z_{\mathrm{ion}}$ and the electric-field direction used by COMSOL.
...
For the one-dimensional model, the normalized ion flux is written in terms of
the electric field used to drive migration:
\begin{equation}
\Gamma_u
...
J--V curve after applying the analysis current-density sign convention.
