...
| thesis presentation workflow graphic | `figures/pptx_ai/workflow_for_comsol_modeling_testing.png` | Chapter 1 COMSOL modeling and testing workflow showing pristine model setup, experimental characterization, calibration, mobile-ion/degradation modeling, and predicted J--V/spatial-profile outputs |
...
| `docs/ml_surrogate_image_prompts.md` + generated image exports | archived conceptual schematic files under `figures/ml_surrogate/` | optional communication figures retained for reference only; Chapter 7 now emphasizes quantitative surrogate plots generated from the analysis pipeline |
...
| `data/processed/comsol/architecture_stress_grid/baseline_pin_stress_matrix_jv_003_pv_metrics_long.csv`; `outputs/tables/comsol_experimental_comparison/xrf_*_metric_points.csv` | `outputs/tables/comsol_experimental_comparison/normalized_pce_*comparison.csv`; `outputs/figures/comsol_experimental_comparison/normalized_pce_*comparison.*` | normalized PCE/PCE0 comparison of COMSOL and experimental light-only, heat-only, and light-plus-heat stress; endpoint figure compares each source at the experimental stress duration |
...
- `scripts/plot_normalized_comsol_experimental_comparison.py`:
  normalized PCE/PCE0 COMSOL-versus-experiment comparison for light-only,
  heat-only, and light-plus-heat stress conditions
