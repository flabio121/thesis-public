...
J--V curve. The modeling challenge is therefore not merely to reproduce the
observed decrease in power-conversion efficiency over time. It is to build a
device model whose
...
multi-rate hysteresis simulations, dark-injection-transient pulse workflows for
mobile-ion concentration extraction, and mechanism-isolation studies. The COMSOL
...
The experimental comparison is qualitative and is framed against multiple
fresh and aged laboratory J--V curves rather than a single-device endpoint.
The accelerated light-plus-heat case reproduces the direction of degradation,
including PCE reduction, fill-factor loss, and J--V knee collapse. The current
implementation demonstrates qualitative mechanism separation, produces
surrogate-ready COMSOL state summaries, and provides a first grouped-validation
surrogate baseline, but experimental calibration remains ongoing. Future work
should improve absolute calibration, validate ion-sensitive parameters with
frozen-ion controls, extend the diagnostic voltage window, implement
light-soaking and self-healing terms, and develop architecture-specific
parameter sets.
